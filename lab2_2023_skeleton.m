...
% Process noise	
 Q=[6.9686*10^(-2) 0 0; %   change prosess noise on Q
    0 1.1004*10^(-2) 0;
    0 0 0.2];
...
stateMeassureLog = zeros(1,nLen);
stateMeassureTHLog = zeros(1,nLen);
...
for n=1:nLen 
	...
	% Compute prediction
    Pp=F*Pe*F.'+G.'*GQ*G; + Q ;     
    %stateMeassureLog(:,n) = [R_axis(row)];
    %stateMeassureTHLog(:,n) = [Th_axis(row)]; 
